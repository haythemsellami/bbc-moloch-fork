...
contract BatchBondedToken is ERC20,ERC20Detailed, BancorFormula {
    ...
    constructor (
        ...
        //category = _category;
        category = Category.BetterForBuyers;
    ...
    function getUserBlocks(address user) public view returns (uint256[] memory) {
    ...
    function addSell(uint256 amount) public returns(bool) {
        ...
        require(burn(msg.sender, amount), "burn must succeed");
        return true;
    }
    function initBatch(uint256 batch) internal {
        clearBatch();
        batches[batch].poolBalance = poolBalance;
        batches[batch].totalSupply = totalSupply();
    ...
    function clearBatch() public {
        ...
        // The totalSupply was decremented when burns took place as the sell orders came in. Now
        ...
        require(mint(address(this), cb.totalBuyReturn), "minting new tokens to be held until buyers collect must succeed");
        cb.cleared = true;
        waitingClear = 0;
    }
    ...
    function claimBuy(uint256 batch, address sender) public {
        ...
        cb.buyers[sender] = 0;
        require(burn(this, individualBuyReturn), "burn must succeed to close claim");
        require(mint(sender, individualBuyReturn), "mint must succeed to close claim");
    }
    ...
    //     require(mint(sender, tokens), "mint must succeed");
    ...
    //     require(burn(msg.sender, sellAmount), "burn must suceed");
    ...
    function mint(address minter, uint256 numTokens) internal returns(bool){
        super._mint(minter, numTokens);
    ...
    function burn(address burner, uint256 numTokens) internal returns(bool) {
        super._burn(burner, numTokens);
